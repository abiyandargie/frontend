...
const DepartmentList = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const data = response.data.departments.map((dep) => ({
          ...
          dp_name: dep.dep_name, // Verify this matches the response data property
          action: (
            <DepartmentButtons
              _id={dep._id}
  ...
  const filterDepartments = (e) => {
    ...
    const records = departments.filter(
      (dep) => dep.dp_name.toLowerCase().includes(query) // Filter by department name
    );
        ...
        <div className="p-5">
          ...
          <div className="flex justify-between items-center">
            <input
              ...
              onChange={filterDepartments} // Pass event handler correctly
            />
            <Link
